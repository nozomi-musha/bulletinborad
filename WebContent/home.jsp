...
<body>
	...
	<Div Align="left">
		<font size=5><c:out value="${user.name}さん         " />
		</font>
		...
		<form action="index.jsp" method="get">
			<label>カテゴリー</label>
				<select name="category">
					<optgroup label="カテゴリーを選択">
					</optgroup>
						<option value=>
						</option>
							<c:forEach items="${categories}" var="category">
								<option value ="${category}"
									<c:if test= "${category == selectCategory}">selected
									</c:if>>
				...
				<br>
				<br>
				<label>日付 <input type="date" name="start" value="${selectStart}">
				</label>
				 <label> ～ <input type="date" name="end" value="${selectEnd}">
				 </label>
	...
	<div class="menu">
		<font color=#0000a0 size =5>【<c:out value="${message.category}" />】
		</font>
		<font size=5><c:out value="${message.title}" />
		</font>
	</div>
	<br>
	...
	<Div Align="right">
		<c:forEach items="${users}" var="user">
			<c:if test="${message.userId==user.id}">
			 ...
			 <fmt:formatDate value="${message.createdAt}" pattern="yyyy/MM/dd HH:mm" />
			 <br>
	...
	<Div Align="center">
		<label for="comment">コメント(500字以内)</label></Div>
			<Div Align="center">
				<textarea name="comment" rows="4" cols="75" wrap="hard">
					<c:if test="${userComment.messageId == message.messageId}">${userComment.text}
					</c:if>
				</textarea>
			</Div>
						<!--裏でmessageIdの取得  -->
	...
	<Div Align="right"><c:out value="投稿者: ${comment.userName}" />
